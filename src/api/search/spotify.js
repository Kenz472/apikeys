    ...
    parseSearch(res) {
        ...
        return {
            ...
            tracks: parse(trackItems, t => ({
                ...
                associations: {
                    audio_count: t.associationsV3?.audioAssociations?.totalCount || 0,
                    video_count: t.associationsV3?.videoAssociations?.totalCount || 0
                },
            ...
            albums: parse(res.albumsV2?.items, a => ({
                ...
                playability: {
                    playable: !!a.playability?.playable,
                    reason: a.playability?.reason || null
                },
            ...
            episodes: parse(res.episodes?.items, ep => ({
                ...
                playability: {
                    playable: ep.playability?.reason === "PLAYABLE",
                    reason: ep.playability?.reason || null
                },
                ...
                podcast: {
                    ...this._getLink(ep.podcastV2?.data?.uri),
                    name: ep.podcastV2?.data?.name || null,
                    publisher: ep.podcastV2?.data?.publisher?.name || null,
                    media_type: ep.podcastV2?.data?.mediaType || null
                }
            })),
    ...
    parseArtist(data) {
        ...
        return {
            ...
            top_tracks: (a.discography?.topTracks?.items || []).map(node => ({
                ...
                album: {
                    ...this._getLink(node.track?.albumOfTrack?.uri),
                    name: node.track?.albumOfTrack?.name || null,
                    images: this._getImg(node.track?.albumOfTrack?.coverArt)
                }
            }))
        };
    }
...
class Spotify {
    ...
    async getToken() {
        try {
            ...
            const {
                data: token
            } = await this.is.get("https://open.spotify.com/api/token", {
                params: {
                    reason: "init",
            ...
            const {
                data: client
            } = await this.is.post('https://clienttoken.spotify.com/v1/clienttoken', {
                client_data: {
    ...
    async query(name, vars) {
        try {
            ...
            const {
                data: res
            } = await this.is.post('https://api-partner.spotify.com/pathfinder/v2/query', {
                variables: vars,
...
module.exports = function (app) {
  app.get("/search/spotify", async (req, res) => {
    ...
    try {
      const searchRes = await spotifyInstance.search(text);

      const tracks = (searchRes?.tracks || []).map(item => ({
        title: item.name,
